...
# 读取训练数据集
train_data = pd.read_csv(r"./train_public.csv")
...
# train_data['work_type'] = label_encoder.fit_transform(train_data['work_type'])
...
ohe = OneHotEncoder()
# train_work_type_oh = ohe.fit_transform(train_data[['work_type']]).toarray()
...
# train_data = pd.concat([train_data, pd.DataFrame(train_work_type_oh, columns=[f'work_type_{i}' for i in range(train_work_type_oh.shape[1])])], axis=1)
...
# 编码分类特征
# test_data['work_type'] = label_encoder.fit_transform(test_data['work_type'])
...
# test_work_type_oh = ohe.transform(test_data[['work_type']]).toarray()
...
# test_data = pd.concat([test_data, pd.DataFrame(test_work_type_oh, columns=[f'work_type_{i}' for i in range(test_work_type_oh.shape[1])])], axis=1)
test_data = pd.concat(
    [
        test_data,
        pd.DataFrame(
